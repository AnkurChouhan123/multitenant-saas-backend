...
public class UserService {
   ...
   // Constructor - Verify all dependencies are injected
	...
	public UserService(UserRepository userRepository, TenantService tenantService,
		...
		this.webhookService = webhookService; // ADD THIS
	}
	...
	public User createUser(User user, Long tenantId) {
		...
		log.info("Step 5.5: Triggering webhook...");
		try {
		...
		try {
			activityLogService.logActivity(tenantId, savedUser.getId(), savedUser.getEmail(),
					savedUser.getFirstName() + " " + savedUser.getLastName(), "User created: " + savedUser.getEmail(),
					"user", "New user added to tenant");
	...
	public User saveWithoutTenant(User user) {
		user.setPassword(passwordEncoder.encode(user.getPassword()));
		return userRepository.save(user);
	}
