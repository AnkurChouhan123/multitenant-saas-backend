...
//      VIEW ACCESS: TENANT_OWNER, TENANT_ADMIN, SUPER_ADMIN
	...
	public ResponseEntity<?> getSubscription(@PathVariable Long tenantId) {
		try {
			...
			try {
				...
				return ResponseEntity.status(HttpStatus.FORBIDDEN).body(createErrorResponse(se.getMessage()));
			}
		}
	}
	...
	public ResponseEntity<?> changePlan(@PathVariable Long tenantId, @RequestParam SubscriptionPlan plan) {
		try {
    ...
    // Cancel subscription MANAGEMENT ACCESS: TENANT_OWNER ONLY
...
//	  Check if subscription is valid VIEW ACCESS: TENANT_OWNER, TENANT_ADMIN,
//	  SUPER_ADMIN
	...
	public ResponseEntity<?> isSubscriptionValid(@PathVariable Long tenantId) {
   	...
   	//
    // Check current user's subscription permissions Returns what actions the user
    // can perform
